...
export default function Page() {
  ...
  const { data: historyData, isLoading: loadingHistory } = useHistoryByUrl(
    routeHistoryUrl || chatUrl
  );
    ...
    askMutation.mutate(
      {
        ...
        historyUrl: activeUrl || undefined,
      },
      {
        onSuccess: (data) => {
  ...
  // Load history when routeHistoryUrl changes
  useEffect(() => {
    if (!routeHistoryUrl) return;
    setChatUrl(routeHistoryUrl);
  }, [routeHistoryUrl]);

  // Update messages when history data is loaded
  useEffect(() => {
    if (historyData) {
      if (historyData.history_id) {
        setHistoryId(historyData.history_id);
        if (historyData.url) {
          setChatUrl(historyData.url);
        }
          ...
          <div className="mt-2">
            <InputGroup>
              <InputGroupTextarea
                ...
                value={input}
                readOnly={askMutation.isPending || !!streamText}
              ...
              <InputGroupAddon align="block-end">
                ...
                <InputGroupButton
                  ...
                  size="icon-sm"
                  onClick={send}
                  disabled={askMutation.isPending || !!streamText}
                >
                  {askMutation.isPending || streamText ? (
